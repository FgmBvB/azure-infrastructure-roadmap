...
Alert Processing Rules modify alerts after they are generated but before notifications are sent.

Common scenarios include:

- Suppressing notifications during planned maintenance.
- Routing alerts to different Action Groups.
- Applying schedules.
- Reducing operational noise.

These rules help prevent unnecessary notifications without disabling monitoring itself.

---

## Smart Groups

Azure Monitor can automatically correlate multiple related alerts into a single incident.
...
Smart Groups are particularly useful during infrastructure-wide failures where many resources generate alerts simultaneously.

> [!TIP]
> Use Alert Processing Rules to control notification behavior while keeping alert generation active.
...
# Analytics Logs vs Basic Logs

Azure Monitor supports different table plans to balance functionality and storage costs.

| Feature | Analytics Logs | Basic Logs |
|----------|----------------|------------|
| KQL Queries | Full | Limited |
| Alert Rules | Supported | Not Supported |
| Interactive Analysis | Yes | Limited |
| Cost | Higher | Lower |
| Typical Usage | Operational monitoring | High-volume diagnostic data |
...
- Debugging data.
- High-volume application traces.
- Logs that are rarely queried.

Basic Logs reduce ingestion costs but provide fewer analytical capabilities.

> [!IMPORTANT]
> Select the appropriate table plan according to operational requirements rather than using Analytics Logs for every data source.
...
Azure Monitor supports two different authorization models for Log Analytics.

## Workspace Context

Users receive permissions directly on the Log Analytics Workspace.

They can:

- Query all available tables.
- Access telemetry from every connected resource.
- Perform cross-resource KQL queries.
...
Azure also supports resource-scoped access.

When a user opens **Logs** from an individual Azure resource (such as a Virtual Machine), Azure automatically limits queries to telemetry generated by that resource.

Benefits include:

- Centralized Log Analytics Workspaces.
- Resource-level isolation.
- Simpler RBAC administration.
- No need to deploy multiple Workspaces for different teams.

> [!IMPORTANT]
> Resource-context access allows organizations to centralize monitoring while maintaining least-privilege access for application owners.
